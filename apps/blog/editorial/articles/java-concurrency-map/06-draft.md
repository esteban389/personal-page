...
Java's basic task interfaces make that separation visible.
[`Runnable`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/Runnable.html)
represents an operation with no result, while
[`Callable<V>`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Callable.html)
returns a value and may throw an exception. Neither interface chooses the thread
that will run it. Their contracts only describe the operation.
...
scheduling. The
[`Executor` contract](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Executor.html)
does not promise asynchronous execution: a minimal executor is allowed to call
`command.run()` in the submitting thread, while other implementations create
threads or reuse them from a pool.

[`ExecutorService`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ExecutorService.html)
adds lifecycle operations and methods such as `submit`, which returns a `Future`.
An application should close or shut down an executor service when it no longer
needs it. In modern Java, the interface is `AutoCloseable`, so a try-with-resources
block can own that lifecycle.

The executable examples target Java 21 or newer because they use virtual threads;
the responsibility map and the older APIs apply more broadly. Here is the
dashboard using one virtual thread per submitted task:
...
code run faster or make timing-dependent access to shared mutable state safe.
[JEP 444 also advises against pooling virtual
threads](https://openjdk.org/jeps/444). Create one per task; use a separate
mechanism when a scarce resource needs a concurrency limit.
...
`Future` also exposes completion status and cancellation. Under the
[`Future` contract](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Future.html),
`cancel(...)` attempts cancellation; already-running work may continue, depending
on timing and whether the task responds to interruption.
...
[Java Language Specification separates these memory actions](https://docs.oracle.com/javase/specs/jls/se25/html/jls-17.html#jls-17.4.2),
...
[`Lock` interfaces](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/locks/Lock.html)
provide explicit locking plus options such as interruptible or timed acquisition
and multiple conditions. That flexibility comes with explicit acquire/release code
and does not make `Lock` an automatic upgrade over `synchronized`.
...
The
[`java.util.concurrent.atomic` package](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/atomic/package-summary.html)
provides thread-safe read-modify-write operations on single variables, including
increment and compare-and-set. One atomic variable does not make a multi-value
invariant atomic. If a rule spans a balance and a ledger entry, or requires several
objects to change together, a lock around the invariant or a design that avoids
shared mutation may fit better.
...
supplies compound operations such as `putIfAbsent`, `compute`, and `merge` for
transitions covered by their contracts:
...
[`computeIfAbsent` implementation](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ConcurrentHashMap.html).

[`BlockingQueue`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/BlockingQueue.html)
solves another shared-data problem: handing items from producers to consumers. Its
operations let a producer fail immediately, wait for space, or wait up to a
timeout; a consumer has corresponding choices when the queue is empty. Capacity
depends on the implementation. `ArrayBlockingQueue` is bounded, while other
implementations may be unbounded or have different handoff semantics.
...
using a semaphore to represent the actual limit on a scarce service.
...
[`java.util.concurrent` synchronizer overview](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/package-summary.html#Synchronizers)
groups these APIs by policy.
...
[`java.util.stream` requirements](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/stream/package-summary.html)
because the mapping is stateless, it does not modify the source, and addition is
associative. Those properties allow the stream implementation to partition work
and combine partial results safely. Stateful operations, encounter-order
requirements, coordination overhead, small inputs, and the available processors
can change the tradeoff. Calling `parallelStream()` does not promise a speedup.
...
- Final title: "How Java's Concurrency APIs Fit Together."
- Keep the parallel-computation section at its reviewed length.
- Keep the plain-`Future` dashboard and the fuller `CompletableFuture` dependency
  graph so their different control-flow roles remain visible.
- Convert official source links to end footnotes in the Astro post.
